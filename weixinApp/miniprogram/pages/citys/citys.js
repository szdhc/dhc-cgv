...
Page({ /** * 生命周期函数--监听页面加载 */
  onLoad: function(options) {
    ...
    this.data.citys = getCityListSortedByInitialLetter();
  ...
  bindSarchInput: function(e) {
    ...
    for (var i = 0; i < citys.length; i++) {
      for (var k = 0; k < citys[i].cityInfo.length; k++)
        if (citys[i].cityInfo[k].city.indexOf(inputVal) == 0 || citys[i].cityInfo[k].short.indexOf(inputVal.toLowerCase()) == 0) {
        // //去除热门城市 
        ...
        for (var j = 0; j < cityResultsTemp.length; j++) {
          if (cityResultsTemp[j] == citys[i].cityInfo[k].city) {
            ifHas = true;
            break;
          }
        }
        if (!ifHas) {
          cityResultsTemp.push(citys[i].cityInfo[k].city);
        }
      }
    }
    this.setData({
      completeList: cityResultsTemp
    })
  },
  // 失焦时清空输入框
  bindBlur: function (e) {
